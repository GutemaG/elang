...
|-------|----------|--------|
| Listening-exercise `audio_url` values use a documented, non-functional placeholder scheme (`https://r2-placeholder.buna.dev/audio/<slug>.mp3`) rather than real Cloudflare R2-hosted audio | Low | **Fixed 2026-09-17** (during `010-offline-caching-and-sync-ui`'s physical-device verification — this is exactly the "before any real device/QA testing" scenario this row was flagged for). `r2-placeholder.buna.dev` doesn't resolve at all, so every listening exercise's audio download failed outright on a real device the moment offline pack-downloading actually tried to fetch it. Swapped to a small, genuinely resolvable public placeholder MP3 (`https://www.kozco.com/tech/piano2-CoolEdit.mp3`) — still a placeholder (not real per-exercise audio), but one that actually works. See `memory-bank/intents/002-core-lesson-loop/units/001-lesson-service/construction-log.md` errata entry. Real R2 audio remains a future pure-data swap, unchanged from the original plan. |
