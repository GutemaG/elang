...
| 2026-09-16 | A "Let's review your mistakes" interstitial now shows exactly once per attempt, right when the retry section of a lesson starts (before the first requeued/previously-missed exercise reappears), instead of the loop-back silently popping back up or (an interim bug) re-showing before every retry | Explicit user follow-up on the missed-question-loop feature: first wanted the loop-back called out rather than invisible, then reported it firing before every retry instead of once, then asked for accurate count-aware copy and a more attractive visual (no image/SVG assets exist in this project, so it's built from Flutter widgets like every other illustration here) | `LessonController` gained one-shot `awaitingRetryIntro`/`startRetryExercise()` gated by a `_retryIntroShown` latch so it fires once per attempt, not once per requeued exercise; `LessonScreen` gained a private `_MistakeReviewCard` interstitial (count-aware copy off `wrongCount`, a layered gradient badge with a count overlay). Client-side only, no backend change. Widget tests cover both the single-mistake and multi-mistake (fires-once) cases; full Flutter suite and analyzer re-run clean |
| 2026-09-16 | Grading now plays a short sound + vibration cue -- a bright chime + single short buzz for correct, a low double-buzz tone + matching double-pulse ("zig zig") vibration for incorrect | Explicit user request: answering should be felt, not just seen, and the two outcomes should never be confusable by feel alone | New `AnswerFeedbackPlayer` interface (mirrors `LessonAudioPlayer`'s plugin-boundary pattern) + `SystemAnswerFeedbackPlayer` (`audioplayers` for the tones). Two short WAV tones generated locally (`assets/sounds/generate_tones.py`, pure stdlib sine synthesis) since the project has no bundled audio/image assets otherwise. `LessonController.check()` fires `playCorrect()`/`playIncorrect()` (fire-and-forget, doesn't block grading). `LessonScreen`/`SkillTreeDashboardScreen`/`LessonDependencies`/`main.dart` all thread the new dependency through the same way `audioPlayer` already does. `FakeAnswerFeedbackPlayer` test double added; new widget test asserts the correct cue fires for each outcome; full suite (81/81) and analyzer re-run clean |
| 2026-09-16 | Vibration switched from `flutter/services.dart`'s `HapticFeedback` to the `vibration` package, and `android.permission.VIBRATE` added to `AndroidManifest.xml` | User reported feeling no vibration at all on a physical device after the first implementation. `HapticFeedback` on Android routes through `View.performHapticFeedback`, which is gated behind the phone's system "touch vibration" setting and can't express a custom multi-pulse pattern -- it can silently no-op depending on the device/setting, and gave no way to produce the requested distinct "zig zig" double pulse. `vibration` talks to the vibration motor directly (`Vibration.hasVibrator()` guard + a `[wait, buzz, wait, buzz]` pattern) so the cue no longer depends on an OS accessibility setting the app can't see | `SystemAnswerFeedbackPlayer` rewritten to call `Vibration.vibrate()`; `AnswerFeedbackPlayer`'s test-facing interface unchanged, so `FakeAnswerFeedbackPlayer` and existing tests needed no changes. Also flagged separately: the "no sound either" half of the report was very likely `pubspec.yaml`'s new `assets:` section not having been picked up by a hot restart on an already-running install -- a full stop-and-`flutter run` (or reinstall) is needed after an assets/dependency change, matching this project's other documented "restart, don't hot-reload" gotchas |
| 2026-09-16 | After a full rebuild, vibration was confirmed working but the sound cue still wasn't audible; switched sound playback from a single `AudioPlayer.play()` call per grading to a preloaded `AudioPool` (one per clip), raised the generated tones' amplitude (0.35 → 0.8), and strengthened vibration (longer pulses, amplitude-control used when the device supports it) per explicit user request to make vibration "a little bit stronger" | `AudioPlayer.play()` loads/decodes the asset fresh on every call, which `audioplayers` itself documents as the wrong shape for "extremely quick firing" SFX and was the most likely source of a silently-lost first playback; `AudioPool` preloads once and reuses low-latency players instead | `SystemAnswerFeedbackPlayer` now lazily creates one `AudioPool` per clip (`PlayerMode.lowLatency`, `maxPlayers: 2`) and calls `pool.start()`; both the sound and vibration paths wrapped in try/catch + `debugPrint` so a real platform failure is visible in the run console instead of silently swallowed by the fire-and-forget `unawaited()` call in `LessonController.check()`. `assets/sounds/generate_tones.py` regenerated both WAVs at the higher amplitude. Interface unchanged; full suite (81/81) and analyzer re-run clean |
