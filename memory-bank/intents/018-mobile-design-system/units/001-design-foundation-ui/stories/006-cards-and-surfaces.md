...
- [ ] **Given** `AppCard`, **When** built, **Then** it is a white face, 2 px border, 24 px radius and `AppShadows.card` (Tactile Level 1)
- [ ] **Given** a `tone` (neutral, primary, secondary, tertiary), **When** set, **Then** border and shelf take that tone's tint, as in the lesson-complete stat cards
- [ ] **Given** `topStripe: true`, **When** set, **Then** a Tibeb stripe runs along the top edge, as on the milestone and refill-timer cards
- [ ] **Given** `onTap`, **When** tapped, **Then** the card presses like a button and exposes a button semantic
- [ ] **Given** `StatCard`, `InfoBanner`, `ListRow` and `SectionHeader`, **When** built, **Then** each matches its mockup (stat cards and the daily-goal banner in lesson complete; list rows follow the FR-11 reference) and appears in the gallery
- [ ] **Given** `SelectableOptionCard`, **When** this story is done, **Then** it is built on `AppCard` with its three states unchanged
