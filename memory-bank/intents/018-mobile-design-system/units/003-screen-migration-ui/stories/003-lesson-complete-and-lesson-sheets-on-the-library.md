...
- [ ] **Given** the lesson-complete screen, **When** shown, **Then** it uses `AppPage` with the celebration background, `StatCard`s, the accuracy `AppCard` with `AppProgressBar`, an `InfoBanner` and a docked Continue, matching its mockups
- [ ] **Given** the exit-lesson, level-up, review-skill and out-of-beans sheets, **When** opened, **Then** each uses `showAppSheet` + `SheetHero`, matching the level-up and out-of-beans mockups; exit and review follow the same pattern
- [ ] **Given** "Not now", "Keep learning" versus "Leave" and similar pairs, **When** shown, **Then** the main action is primary, a real alternative is secondary and a dismissal is a text link
- [ ] **Given** the lesson-complete and sheet tests, **When** run, **Then** all pass, changed only for replaced types, and each sheet returns the same result as before
